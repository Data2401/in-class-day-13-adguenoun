...
my_flights <- my_flights %>% 
  mutate(NetTime =  dep_delay - arr_delay)
```
...
```{r}
my_flights <- my_flights %>% arrange(desc(NetTime))
```
...
mean(my_flights$NetTime, na.rm = TRUE)
...
hou_flights <- filter(my_flights, dest == "HOU") %>% select(origin, dest, NetTime)
...
```{r}
mean(hou_flights$NetTime, na.rm = TRUE) 
...
```{r}
my_flights %>% filter(origin == 'JFK',dest == 'SEA') %>% select(air_time) %>% na.omit() %>%  
...
```{r}
# count characters from each unique planet

starwars %>% group_by(homeworld) %>% count(name) %>% unique() 
